/**
 * Browser-owned active-page sampling for the prompt request's `browserPage`
 * field. The dsh browser extension's side panel posts the user's active tab
 * into the embedded app frame on every change; the newest accepted sample
 * rides the next prompt like the browser time zone does. Web and file tabs
 * sample as pages; any other scheme samples as `null` ("no active page"), so
 * a closed page never lingers as stale context. Outside the extension no
 * message ever arrives and every prompt omits the field.
 */

// Value import from the inline-safe wire layer (not the connection plugin):
// plugin-to-plugin value imports are a bundle purity error.
import { BROWSER_PAGE_TITLE_MAX_CHARS, BROWSER_PAGE_URL_MAX_CHARS } from '../types.ts'
