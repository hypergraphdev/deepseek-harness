...
export const InputBar = memo(function InputBar({
  ...
  // Dictation inserts each finalized segment at the editor selection through
  // the paste path, so undo and reference sanitizing match typed input; one
  // space separates it from a draft that does not already end in whitespace.
  const appendDictation = useCallback((text: string): void => {
    ...
    keyboard.paste(draft === '' || /\s$/.test(draft) ? text : ` ${text}`)
